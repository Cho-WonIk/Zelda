...
void UZCMaterialStateComponent::BeginPlay()
{
	...
	WordSubsystemHandle = WorldSubsystem->OnAssetLoadFinished.AddLambda([&]() {
		MaterialData = WorldSubsystem->GetMaterialInstanceData(Material);
		CurrentElementState = FElementState::Empty;
		//UZCLogger::Warning(TEXT("월드 서브 시스템 상태 : {0}, 물질 상태 : {1}"), WorldSubsystem ? TEXT("있음") : TEXT("없음"), MaterialData ? TEXT("있음") : TEXT("없음"));
	});
}
...
void UZCMaterialStateComponent::ApplyElementExposure(const FElementInfo& NewElementInfo)
{
	...
	// 5. 확산 될때 SpreadingCount를 증가시키며 FElementCDO에 정의된 값을 넘어서 전파될 수 없다(확산 중단)
	...
	//const bool bIsSpreadInput = (NewElementInfo.SpreadCount >= 0);
	const int32 NewSpreadCount = NewElementInfo.SpreadCount + 1;
	...
	if (!CurrentElementState.IsEmpty())
	{
		if (CurrentElementState.ElementTag == NewElementInfo.ElementTag && CurrentElementState.SpreadingCount < NewElementInfo.SpreadCount)
		{
			return;
		}
		// 불 -> 불 과 같은 로직을 안만들면 역전파가 안일어남 + 재 갱신 안일어남
		if (WorldSubsystem->TryGetOutCome(NewElementInfo.ElementTag, CurrentElementState.ElementTag, Out))
		{
			// 상쇄(EmptyTag)
			...
			ApplyElement(Out, NewSpreadCount);
	...
	{
		if (!WorldSubsystem->TryGetOutCome(NewElementInfo.ElementTag, Material, Out)) return;
		// 상쇄(EmptyTag)면 적용 없음
...
{
	ElementData = WorldSubsystem->GetElementInstanceData(ReactionResult.NewElementTag);
	if (!ElementData) return;

	// 전파 한계 검사
	if (SpreadingCount >= ElementData->MaxSpreadingCount)
	{
		UZCLogger::Warning(TEXT("{0}에 원소 반응이 트리거되었으나 확산 한계치로 인해 실패"), *OwnerCasted->GetName());
		return; // 더 이상 전파 불가
	}
	...
	CurrentElementState.ElementTag = ReactionResult.NewElementTag;
	CurrentElementState.Duration = ReactionResult.Duration;
	CurrentElementState.bIsDamageOnce = ReactionResult.bIsDamageOnce;
	CurrentElementState.ElementTickDamage = ReactionResult.ElementTickDamage;
	CurrentElementState.SpreadingCount = SpreadingCount;

	if (CurrentElementState.bIsDamageOnce)
	{
		// TODO: 초기 데미지가 있는 경우 Owner 액터에 데미지 주입, 액터에게 데미지 전달 로직 지금은 보류
	}
...
void UZCMaterialStateComponent::ProcessElementSpreading()
{
	...
	// 범위 내 액터들 엘리먼트 임계치 증가 시키고, 데미지 옵션이 true인경우 데미지 전달
	FElementInfo NewInfo(CurrentElementState.ElementTag, CurrentElementState.Duration, CurrentElementState.SpreadingCount);
	float ApplyDamage = CurrentElementState.bIsDamageOnce ? 0.0f : CurrentElementState.ElementTickDamage;
...
void UZCMaterialStateComponent::StartFX()
{
...
void UZCMaterialStateComponent::EndFX()
{
