...
class UZCPhysActorMovementComponent;
...
enum class EOverlayState : uint8
{
	...
	All = ItemHighlight | UltraHand | SelectUltraHand,
};
...
protected:
	...
	// 울트라핸드 그랩시 상태 업데이트 로직
	void RefreshAssembleCandidates();

	void UpdateOverlayMaterial();
	void SetOutlineMaterial(UMaterialInterface* NewMaterial);

protected:
	// 액터가 받은 원소 정보만을 하위클래스에서 알아야할따 사용
	virtual void GetElement(const FGameplayTag& ElementTag) {}
...
protected:
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Zelda|Actor", meta = (Bitmask, BitmaskEnum = "EOverlayState", DisplayName = "오버레이 머티리얼 표시 가능 여부"))
	uint8 CanShowOverlayState = static_cast<uint8>(EOverlayState::All);
