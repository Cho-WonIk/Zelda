...
public:
	...
	// 현재 Socekt Offset에서 추가할 오프셋값으로 변경
	void ChangeSocketOffsetDelta(const FVector& NewOffset) { TargetSocketOffset = SocketOffset + NewOffset; }
	// DefaultSocketOffset으로 복원
	void RestoreSocketOffset() { TargetSocketOffset = DefaultSocketOffset; }
...
protected:
	...
	// SocketOffset이 변경될 때 보간 속도
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "락온 설정", meta = (AllowPrivateAccess = "true", DisplayName = "소켓 오프셋 변경 속도"))
	float SocketOffsetInterpSpeed = 10.0f;
