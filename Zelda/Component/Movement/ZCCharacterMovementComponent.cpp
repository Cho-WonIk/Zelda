...
void UZCCharacterMovementComponent::OnMovementUpdated(float DeltaSeconds, const FVector& OldLocation, const FVector& OldVelocity)
{
	...
	else if (bWantsToRidingGear)
	{
		SetMovementMode(EMovementMode::MOVE_Custom, ECustomMovementMode::CMOVE_RidingGear);
	}
}
...
void UZCCharacterMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
	...
	if (IsRidingGear())
	{
		OnMovementChangeDelegate.Broadcast(EMovementMode::MOVE_Custom, ECustomMovementMode::CMOVE_RidingGear);

		if (CurrentRidingGear.IsValid())
		{
			auto* Cap = CharacterOwner->GetCapsuleComponent();

			FTransform SocketTransform = CurrentRidingGear->GetRootComponent()->GetSocketTransform(GearAttachSocketName);
			// 위치와 회전 정보만 추출 (스케일 무시)
			FVector SocketWorldLocation = SocketTransform.GetLocation();
			FRotator SocketWorldRotation = SocketTransform.GetRotation().Rotator();
			FVector SocketUpVector = FRotationMatrix(SocketWorldRotation).GetUnitAxis(EAxis::Z);

			// 월드 기준 오프셋 계산 (스케일 영향 없음)
			float CapsuleHalfHeight = Cap->GetScaledCapsuleHalfHeight();

			// 소켓 위치에서 소켓의 위쪽 방향으로 AbsoluteOffset만큼 이동
			FVector TargetWorldLocation = SocketWorldLocation + (SocketUpVector * CapsuleHalfHeight);

			CharacterOwner->SetActorLocationAndRotation(TargetWorldLocation, SocketWorldRotation, false, nullptr, ETeleportType::ResetPhysics);
			...
			CharacterOwner->AttachToActor(CurrentRidingGear.Get(), AttachRules, GearAttachSocketName);
	...
	const bool bWasRidingGear = PreviousMovementMode == MOVE_Custom && PreviousCustomMode == CMOVE_RidingGear;
	if (bWasRidingGear)
	{
		OnMovementChangeDelegate.Broadcast(EMovementMode::MOVE_Custom, ECustomMovementMode::CMOVE_MAX);

		if (auto* Cap = CharacterOwner->GetCapsuleComponent())
		{
			// 하차 후에는 다시 물리 상호작용이 가능하도록 Query와 Physics를 모두 켭니다.
			Cap->SetCollisionEnabled(ECollisionEnabled::QueryAndPhysics);
		}

		// 부착 해제
		FDetachmentTransformRules DetachRules(EDetachmentRule::KeepWorld, true);
		CharacterOwner->DetachFromActor(DetachRules);

		CurrentRidingGear = nullptr;

		GearAttachSocketName = NAME_None;
	}
...
void UZCCharacterMovementComponent::PhysCustom(float DeltaTime, int32 Iterations)
{
	...
	if (CustomMovementMode == ECustomMovementMode::CMOVE_RidingGear)
	{
		PhysRidingGear(DeltaTime, Iterations);
	}

	Super::PhysCustom(DeltaTime, Iterations);
}

void UZCCharacterMovementComponent::PhysWalking(float deltaTime, int32 Iterations)
{
...
/* ======================================= 기어 탑승 로직들 ========================================= */
bool UZCCharacterMovementComponent::CanStartRidingGear()
{
	return MovementMode == EMovementMode::MOVE_Walking && bCanRideGear && UpdatedComponent;
}

void UZCCharacterMovementComponent::TryRidingGear(AActor* TargetGear, FName Socket)
{
	if (TargetGear && CanStartRidingGear())
	{
		bWantsToRidingGear = true;
		CurrentRidingGear = TargetGear;
		GearAttachSocketName = Socket;
	}
}

void UZCCharacterMovementComponent::CancelRidingGear()
{
	bWantsToRidingGear = false;
}

void UZCCharacterMovementComponent::PhysRidingGear(float DeltaTime, int32 Iterations)
{
	if (DeltaTime < MIN_TICK_TIME) return;

	if (ShouldStopRidingGear())
	{
		StopRidingGear(DeltaTime, Iterations);
		return;
	}
...
bool UZCCharacterMovementComponent::ShouldStopRidingGear() const
{
	// TODO : 추후 강제 기어 하차 로직 구현
	return !bWantsToRidingGear;;
}

void UZCCharacterMovementComponent::StopRidingGear(float DeltaTime, int32 Iterations)
{
	bWantsToRidingGear = false;
...
void UZCCharacterMovementComponent::PhysGliding(float DeltaTime, int32 Iterations)
{
	...
	while ((RemainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations))
	{
		...
		if (Hit.IsValidBlockingHit())
		{
			...
			// 슬라이드 후에도 위치가 변경되지 않았다면, 강제로 종료
			if ((UpdatedComponent->GetComponentLocation() - OldLocation).SizeSquared() <= KINDA_SMALL_NUMBER)
			{
				// 글라이딩을 종료하고 Falling 혹은 클라이밍 상태로 전환
				StopGliding(RemainingTime, Iterations);
				break;
			}
		}
	}
...
{
	// Z축 방향 속도 제거
	FVector HorizontalVelocity = ProjectToGravityFloor(Velocity + FallAcceleration * DeltaTime).GetClampedToMaxSize(MaxGlidingSpeed);

	// Z축 중력 적용 및 제한
	const FVector GravityDir = -GetGravityDirection();
	const float VerticalSpeed = FVector::DotProduct(Velocity, GravityDir);
