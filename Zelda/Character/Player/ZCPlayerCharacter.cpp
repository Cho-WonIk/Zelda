...
void AZCPlayerCharacter::BeginPlay()
{
	...
	ZCPlayerController->SetPlayerVFX(GetZCNiagaraComponent());
}
...
void AZCPlayerCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent))
	{
		...
		// 기어 조작 무브먼트
		EnhancedInputComponent->BindAction(RideMoveAction, ETriggerEvent::Triggered, this, &AZCPlayerCharacter::RideGearMove);
	}
}
...
void AZCPlayerCharacter::Look(const FInputActionValue& Value)
{
	...
	//UZCLogger::Warning(TEXT("{0}"), LookAxisVector);
...
void AZCPlayerCharacter::Jump()
{
	...
	if (bIsCrouched)
	{
		...
		if (EnumHasAnyFlags(CurrentDirection, EZCDirection::Forward))
			PlayAnimMontage(CrouchEvasion[EZCDirection::Forward]);
		else if (EnumHasAnyFlags(CurrentDirection, EZCDirection::Backward))
			PlayAnimMontage(CrouchEvasion[EZCDirection::Backward]);
		else if (EnumHasAnyFlags(CurrentDirection, EZCDirection::Left))
			PlayAnimMontage(CrouchEvasion[EZCDirection::Left]);
		else if (EnumHasAnyFlags(CurrentDirection, EZCDirection::Right))
	...
	if (ZCCharacterMovementComponent->IsMovingOnGround())
	{
		...
		{
			if (EnumHasAnyFlags(CurrentDirection, EZCDirection::Backward))
...
void AZCPlayerCharacter::SetRideGear(AZCInputGearActor* GearActor)
{
	CurrentRideGear = GearActor;

	ZCCharacterMovementComponent->SetCanRideGear(GearActor != nullptr);
}

void AZCPlayerCharacter::RideOnGear()
{
	ZCCharacterMovementComponent->TryRidingGear(CurrentRideGear, CurrentRideGear->GetRideSocketName());

	CurrentRideGear->RideStart();
}

void AZCPlayerCharacter::RideOffGear()
{
	ZCCharacterMovementComponent->CancelRidingGear();

	CurrentRideGear->RideEnd();
}
...
/*==================기어 라이딩 로직 ===================*/
void AZCPlayerCharacter::RideGearMove(const FInputActionValue& Value)
{
	FVector2D MoveVector = Value.Get<FVector2D>();

	if (!CurrentRideGear) return;

	CurrentRideGear->GetPlayerInput(MoveVector);
}
...
EZCDirection AZCPlayerCharacter::GetInputDirection() const
{
	...
	if (Input.IsNearlyZero()) return EZCDirection::None;
	...
	EZCDirection Result = EZCDirection::None;

	// 앞/뒤 판정 (임계값 0.382683f = cos(67.5도))
	if (ForwardDot > 0.382683f)       Result |= EZCDirection::Forward;
	else if (ForwardDot < -0.382683f)  Result |= EZCDirection::Backward;

	// 왼쪽/오른쪽 판정
	if (RightDot > 0.382683f)        Result |= EZCDirection::Right;
	else if (RightDot < -0.382683f)   Result |= EZCDirection::Left;

	// 만약 대각선 임계값에 걸리지 않았지만 입력은 있는 경우 보정 (상하좌우 4방향 우선순위)
	if (Result == EZCDirection::None)
	{
		if (FMath::Abs(ForwardDot) > FMath::Abs(RightDot))
			Result = (ForwardDot > 0) ? EZCDirection::Forward : EZCDirection::Backward;
		else
			Result = (RightDot > 0) ? EZCDirection::Right : EZCDirection::Left;
	}
